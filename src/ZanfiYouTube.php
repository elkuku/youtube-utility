...
class ZanfiYouTube
{
    ...
    function __construct($vId = null)
    {
        ...
        (isset($this->arr_Video_info['streamingData']['adaptiveFormats']))
            ? $this->arr_Video_adaptive_formats = $this->arr_Video_info['streamingData']['adaptiveFormats']
            : $this->arr_Video_adaptive_formats = [];

        $this->check_url_signed($this->arr_Video_adaptive_formats);

        (isset($this->arr_Video_info['streamingData']['formats']))
            ? $this->arr_Video_formats = $this->arr_Video_info['streamingData']['formats']
            : $this->arr_Video_formats = [];

        $this->check_url_signed($this->arr_Video_formats);

        (isset($this->arr_Video_info['videoDetails']['thumbnail']))
            ? $this->arr_Video_thumbnails = $this->arr_Video_info['videoDetails']['thumbnail']
            : $this->arr_Video_thumbnails = [];
    }
    ...
    function get_video_id($str)
    {
        ...
        if (strpos($str, '://') !== false) {
            ...
            if (isset($my_array_of_vars['vi'])) {
                return ($my_array_of_vars['vi']);
            }
        } else {
            return ($str);
        }
        ...
        foreach ($this->arr_Video_formats as $val) {
            foreach ($this->arrBestBoth as $bb) {
                if ($val['itag'] == $bb) {
                    ...
                    $this->download(
                        $val['url'],
                        $this->arr_Video_info['videoDetails']['title'].'.'
                        .$arrExt[1]
                    );
    ...
    {
        $this->youtube_html = file_get_contents(
            'https://m.youtube.com/watch?v='.$this->video_id
        );

        $this->get_base_js();

        $scr = $this->get_string_between(
            $this->youtube_html,
    ...
    function get_base_js()
    {
        ...
        $this->base_js = file_get_contents(
            'https://www.youtube.com'.$matches[0][1].'base.js'
        );
    }
    ...
    public function parseFunctionName()
    {
        if (preg_match(
            ...
            if (preg_match(
                '@(?:\b|[^a-zA-Z0-9$])([a-zA-Z0-9$]{2,3})\s*=\s*function\(\s*a\s*\)\s*{\s*a\s*=\s*a\.split\(\s*""\s*\)@is',
                $this->base_js,
                $matches
            )
            ) {
    ...
    function downloadProgress(
        ...
        ob_start();
        echo 'download_size: '.$download_size.'; downloaded_size: '
            .$downloaded_size.';<br>';
        ob_flush();
        ob_end_flush();
    }
    ...
    function check_url_signed(&$arrVideo)
    {
        foreach ($arrVideo as $key => $val) {
            if (!isset($val['url'])
                || (isset($val['url']) && $val['url'] == ''
                    && isset($val['signatureCipher']))
            ) {
